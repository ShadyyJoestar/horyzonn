...
// Pertahanan untuk kemungkinan C: profile_tags bisa string/array/null
...
export async function runAcademicAssessment(): Promise<{ error: string } | void> {
  let targetId = "";
  ...
  try {
    ...
    // Normalisasi profile_tags sebelum masuk engine (cegah kemungkinan C)
    const safePaths = (paths || []).map((p) => ({
      ...p,
      profile_tags: parseTags(p.profile_tags),
    }));

    const result = runAcademicClassification(userProfile, safePaths, competencyNames);
    ...
    if (error || !row) {
      return { error: `Failed to save classification: ${error?.message || "unknown"}` };
    }

    targetId = row.id;
  } catch (e) {
    // Redirect / notFound error Next.js harus dilempar ulang, jangan ditelan
    if (
      typeof e === "object" &&
      e !== null &&
      "digest" in e &&
    ...
    return { error: e instanceof Error ? e.message : "Unexpected error while classifying" };
  }

  // redirect HARUS di luar try/catch (per official Next.js docs)
  revalidatePath("/dashboard");
  revalidatePath("/dashboard/academic");
  redirect(`/dashboard/academic/${targetId}`);
}